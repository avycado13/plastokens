...
def test_transfer(client):
    ...
    response = client.post(url_for('security.login'), data={
        'email': 'test@me.com',
        'password': 'password'
    ...
    # Perform transfer
    response = client.post(url_for('main.index'), data={
        'recipient': 'test@you.com',
        'amount': 100
    }, follow_redirects=True)
    assert response.status_code == 200
    ...
    transaction = Transaction.query.filter_by(sender_id=sender.id, recipient_id=recipient.id).first()
